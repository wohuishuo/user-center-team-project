...
用 VitePress 搭建的教学网站，定位:**通过用户中心一个项目，同时学会现代技术栈、软件工程方法论与团队协作**。

**写作规范(重要)**:全书遵循「金字塔 / 一分钟说话」方法——每段**结论先行**,再给根据、例子,写完检查逻辑链;用「我们」不用「我」,读者中心。**不做 2022↔2026 过程性技术对比**,直接讲现在该怎么做(省篇幅讲具体细节)。规范正文见 `web-book/docs/guide/writing-style.md`。理论篇已从 11 章精简合并为 6 章。
